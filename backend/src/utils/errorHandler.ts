...
export const handleError = async (
  ...
  try {
    ...
    const userId = (request as { momentumUser?: { id: string } }).momentumUser?.id || null;
    ...
    // Ship 5xx to Sentry when DSN configured (no-op otherwise).
    if (statusCode >= 500) {
      try {
        const wallet = (request as { momentumUser?: { walletAddress?: string } }).momentumUser
          ?.walletAddress;
        captureError(originalError ?? new Error(`[${errorCode}] ${message}`), {
          errorCode,
          statusCode,
          route: requestInfo.path ?? undefined,
