  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      ...
      cells.forEach((cell) => {
        ...
        const accentWave1 = Math.sin(
          time * 0.015 + cell.row * 0.4 + cell.col * 0.3,
        )
        const accentWave2 = Math.sin(
          time * 0.012 - cell.col * 0.5 + cell.row * 0.2,
        )
        const accentWave3 = Math.sin(
          time * 0.018 + (cell.row + cell.col) * 0.25,
        )
        const targetAccent =
          Math.max(0, (accentWave1 + accentWave2 + accentWave3) / 3 - 0.3) * 2
        ...
        const wave2 = Math.sin(
          time * cell.speed * 0.7 + cell.phase + Math.PI / 3,
        )
        ...
        const r = Math.round(
          baseColor[0] + (accentColor[0] - baseColor[0]) * cell.accentAmount,
        )
        const g = Math.round(
          baseColor[1] + (accentColor[1] - baseColor[1]) * cell.accentAmount,
        )
        const b = Math.round(
          baseColor[2] + (accentColor[2] - baseColor[2]) * cell.accentAmount,
        )
